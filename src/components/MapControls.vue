      ...
      <div class="select-wrapper">
        ...
        >
          <ListboxOptions class="map-options">
            <ListboxOption v-for="m in modes" :key="m.id" :value="m.id" class="map-option">
              {{ m.name }}
            </ListboxOption>
          </ListboxOptions>
        </transition>
      ...
      <div class="select-wrapper">
        ...
        >
          <ListboxOptions class="map-options wide">
            <ListboxOption v-for="s in styles" :key="s.id" :value="s.id" class="map-option">
              {{ s.name }}
            </ListboxOption>
          </ListboxOptions>
        </transition>
...
watch(
  () => props.modelMode,
  v => (internalMode.value = v)
)
watch(
  () => props.modelStyle,
  v => (internalStyle.value = v)
)
...
.listbox-button.map-control {
  @extend .map-control;
}
.listbox-options.map-options {
  @extend .map-options;
}
.listbox-option.map-option {
  @extend .map-option;
}
.chev {
  font-size: 12px;
